# =============================================================================
# v2 REFERENCE — NOT wired into v1. Do not import from the matador package.
#
# This is the serve/return (Barnett-Clarke) -> point-by-point Markov match
# model that our design defers to v2. v1 uses surface Elo -> logistic directly
# (see DESIGN-DECISIONS.md "Fair-value model approach"), so keep this here as a
# reference to build v2 from / to A-B against Elo *after* the baseline's edge is
# measured. Provenance: a phone Claude session; full brief in
# Research/tennis_bot_spec.md.
#
# Kalshi caveats when this is eventually used:
#   - devig_multiplicative / devig_shin apply ONLY to sportsbook proxy odds
#     (Pinnacle/Bet365 via tennis-data.co.uk) in the Phase-6 backtest. Live
#     Kalshi prices are the implied probability with NO vig to strip.
#   - edge_and_stake / scan_side use decimal-odds EV and do NOT subtract Kalshi
#     fees. The live path uses matador's net-of-fee binary-contract Kelly
#     instead (net_edge = (p_model - price) - fee*price*(1-price)).
# =============================================================================
"""
tennis_edge.py
==============
